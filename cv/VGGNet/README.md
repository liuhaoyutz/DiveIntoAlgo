...
5、梯度检查  
如果所有参数的梯度都为 0，会导致模型无法更新参数，从而无法学习。这种情况通常被称为梯度消失（Gradient Vanishing），是深度学习中常见的问题之一。解决方法包括改进初始化、使用合适的激活函数、调整学习率等。  
```python
...
梯度消失和爆炸的区别  
特性            梯度消失                                    梯度爆炸  
梯度值           非常小（接近于 0）                           非常大（甚至无穷大或 NaN）  
损失值           几乎不下降                                  突然变得非常大或变为 NaN   
参数值           几乎不更新                                  变得非常大或出现 NaN   
原因             深层网络、不合适的激活函数（如 Sigmoid）       学习率过高、权重初始化不当、网络过深  
解决方法          使用 ReLU、残差连接、批量归一化               梯度裁剪、降低学习率、权重初始化  
